...
SOCP 求解得到的最优解可能落在二阶锥的**内部**而非表面，此时松弛不紧，解不对应物理可行的交流潮流解。对每条支路 $(i,j)$、每个时段 $t$，定义**锥松弛间隙**（SOCP relaxation gap）$\Delta_{ij,t}$：

$$
\Delta_{ij,t} = \frac{(U_{i,t}+U_{j,t})^2 - [4T_{ij,t}^2 + 4Z_{ij,t}^2 + (U_{i,t}-U_{j,t})^2]}{(U_{i,t}+U_{j,t})^2}
$$

$\Delta_{ij,t} \in [0,1]$，越接近 0 表示松弛越紧。间隙 $\Delta_{ij,t} \approx 0$ 时松弛是紧的，SOCP 解可直接恢复为交流潮流物理解；$\Delta_{ij,t} > \epsilon$（收敛容差）时需进行**锥还原**。
...
| $\Delta_{ij}^{\max} = \max_t \Delta_{ij,t}$ | 各支路最差时段松弛 |

$\max \Delta_{ij,t}$ 小于 $\epsilon$（取 $10^{-4}$）视为严格紧；$\epsilon$ 到 $10^{-2}$ 为近似紧；超过 $10^{-2}$ 需重点关注，必要时进行锥还原。
...
当 SOCP 求解完成后，若锥松弛间隙 $\max \Delta_{ij,t} > \epsilon$，则需要进行**锥还原（Cone Recovery）**，将解从锥的内部驱动到锥面上。锥还原方法基于 **Cauchy-Schwarz 不等式**，导出线性的**方向割约束**来收紧可行域。

对于支路 $(i,j)$，记锥表示向量 $\mathbf{x}_{ij} = (2T_{ij,t}, 2Z_{ij,t}, U_{i,t}-U_{j,t})^T$，二阶锥约束为 $\|\mathbf{x}_{ij}\|_2 \leq U_{i,t} + U_{j,t}$。

对任意方向向量 $\mathbf{d} = (d_T, d_Z, d_U)^T$，由 Cauchy-Schwarz 不等式得：

$$
\frac{\mathbf{d}^T \mathbf{x}_{ij}}{\|\mathbf{d}\|_2} \leq \|\mathbf{x}_{ij}\|_2 \leq U_{i,t} + U_{j,t}
$$

在方向 $\mathbf{d}$ 上添加**带收缩因子的方向割约束**：

$$
\frac{\mathbf{d}^T \mathbf{x}_{ij}}{\|\mathbf{d}\|_2} \geq r \cdot (U_{i,t} + U_{j,t}), \quad r \in [0, 1]
$$
...
以初始 SOCP 解 $(T^*, Z^*, U^*)$ 为出发点，对每条支路 $(i,j)$ 提取当前锥表示向量方向 $\mathbf{d} = (2T^*_{ij,t}, 2Z^*_{ij,t}, U^*_{i,t}-U^*_{j,t})^T$，计算初始收缩因子 $r_{\min} = \|\mathbf{d}\| / (U^*_{i,t} + U^*_{j,t})$，将对应的方向割约束加入约束集合 $\mathcal{L}$，置层计数 $\ell=1$、迭代计数 $k=0$、间隙历史集合 $\mathcal{H}=\emptyset$。

**阶段二：主迭代循环**

每轮迭代包含以下步骤：

- **步骤 1（SOCP 求解）**：在当前割约束集合 $\mathcal{L}$ 下求解 SOCP，得到新解 $(T^k, Z^k, U^k)$
- **步骤 2（间隙计算）**：对所有支路和时段计算锥松弛间隙 $\Delta^k_{ij,t}$，取最大间隙 $\Delta^k_{\max}$ 记录至历史集合 $\mathcal{H}$
- **步骤 3（全局收敛检验）**：若 $\Delta^k_{\max} \leq \epsilon$，则所有支路的松弛均已收紧，返回当前解并标记为收敛
- **步骤 4（层内自适应方向更新）**：对仍有间隙（$\Delta^k_{ij,t} > \epsilon$）的支路，以当前解方向 $\hat{\mathbf{d}} = (2T^k_{ij,t}, 2Z^k_{ij,t}, U^k_{i,t}-U^k_{j,t})^T / \|(2T^k_{ij,t}, 2Z^k_{ij,t}, U^k_{i,t}-U^k_{j,t})\|$ 和当前层收缩因子 $r_\ell$ 生成新的方向割，补充进 $\mathcal{L}$。收缩因子按层线性递增：$r_\ell = r_{\min} + (\ell-1)/(L_{\max}-1) \cdot (1 - r_{\min})$
...
while k < K_max:
    k ← k + 1
    求解 SOCP（含当前 L 中所有割约束）→ 得到 (T^k, Z^k, U^k)
    ↓
    计算所有支路时段的锥松弛间隙 Δ^k_{ij,t}
    Δ^k_max ← max_{ij,t} Δ^k_{ij,t}；将 Δ^k_max 写入历史集合 H
    ↓
    若 Δ^k_max ≤ ε → 返回当前解，标记 Converged ✓
    ↓
    层内自适应：对有间隙的支路，以当前方向 d=(2T^k_{ij,t}, 2Z^k_{ij,t}, U^k_{i,t}-U^k_{j,t})^T 和 r_ℓ 生成新割，加入 L
    ↓
...
**时间复杂度**：设决策变量数 $n = O(T \cdot |E|)$，第 $k$ 次迭代时约束集合 $\mathcal{L}$ 已累积至多 $k \cdot T|E|$ 条割，对应 SOCP 求解代价为 $O(k \cdot n^3)$。全部 $K_{\max}$ 次迭代的总代价为：
...
**典型迭代次数**：实践中 10--30 次迭代即可收敛；硬上界为 $K_{\max}$（算法强制终止条件）。
...
for scenario in selected_scenarios:
    ...
    [求解成功]
        → 提取变量最优值（.X）
        → 计算锥松弛间隙 Δ_arr（逐支路逐时段）
        → 打印松紧度报告
        ↓
        [若 max Δ > ε：松弛不紧，启动锥还原]
            → cone_recovery(T*, Z*, U*, L={})  # 初始化方向割集合
            → while k < K_max:
                  添加方向割约束到模型，重新求解 SOCP
                  计算新的 Δ^k_arr；若 max Δ^k ≤ ε → 收敛，退出
                  检测停滞 → 层次提升或退出
            → 返回还原解（T^k, Z^k, U^k），标记收敛状态
        [若 max Δ ≤ ε：松弛已紧，直接使用 SOCP 解]
        ↓
        → save_results(...)               # 写 .npz + 追加 summary.csv
        → visualize_results(...)          # 生成图表（含 socp_gap.png）
    [求解失败]
        → computeIIS() + 写 .ilp 文件
        → 打印失败原因，继续下一场景
    ↓
