...
\STATE $r_{\min} \leftarrow \dfrac{\|\mathbf{d}\|}{\sqrt{l^*_{t,i,j}v^*_{t,i}}}$ \COMMENT{初始收缩因子}
\end{algorithmic}
\end{algorithm}
...
\WHILE{$k < K_{\max}$}
    ...
    \ENDFOR
    \STATE $\Delta^k_{\max} \leftarrow \max_{t,i,j} \Delta^k_{t,i,j}$
    \STATE $\mathcal{H} \leftarrow \mathcal{H} \cup \{\Delta^k_{\max}\}$
...
\IF{$|\mathcal{H}| \geq W_{\text{stag}}$}
    \STATE $\text{recent} \leftarrow \text{LAST\_N\_ELEMENTS}(\mathcal{H}, W_{\text{stag}})$
    \STATE $\text{improvements} \leftarrow \{\text{recent}[i] - \text{recent}[i+1] \mid i = 1,\ldots,W_{\text{stag}}-1\}$
